...
| | RecursiveChunker (`recursive`) | 10 | 289 | ✅ Tách theo `\n\n` → giữ nguyên mục |
| dieukhoandatcoc.md (5,099 chars) | FixedSizeChunker (`fixed_size`) | 12 | 471 | ❌ Cắt giữa điều khoản |
| | SentenceChunker (`by_sentences`) | 12 | 421 | ⚠️ Giữ câu, nhưng gộp lẫn điều khoản |
| | RecursiveChunker (`recursive`) | 16 | 317 | ✅ Mỗi chunk ~ 1 điều khoản |
| chinhsachthuepin.md (14,893 chars) | FixedSizeChunker (`fixed_size`) | 33 | 500 | ❌ Cắt giữa điều, khoản |
| | SentenceChunker (`by_sentences`) | 38 | 390 | ⚠️ Tách câu tốt, nhưng mất header |
| | RecursiveChunker (`recursive`) | 44 | 337 | ✅ Tách theo paragraph → giữ cấu trúc |

### Strategy Của Tôi

**Loại:** RecursiveChunker (Strategy 3)

**Mô tả cách hoạt động:**
> RecursiveChunker thử tách text bằng các separator theo thứ tự ưu tiên: `\n\n` → `\n` → `. ` → ` ` → `""`. Đầu tiên, nó cố gắng tách theo paragraph (`\n\n`) — đây là ranh giới tự nhiên nhất trong văn bản pháp lý. Nếu một paragraph vẫn quá dài, nó tiếp tục đệ quy với separator tiếp theo (`\n`, rồi `. `). Các phần nhỏ được merge lại cho đến khi đạt gần `chunk_size`, đảm bảo mỗi chunk chứa nội dung trọn vẹn nhất có thể.

**Tại sao tôi chọn strategy này cho domain nhóm?**
> Tài liệu pháp lý VinFast có cấu trúc rõ ràng: mỗi điều khoản được ngăn cách bằng dòng trống (`\n\n`), các mục con phân tách bằng `\n`. RecursiveChunker khai thác đúng cấu trúc này — ưu tiên tách theo paragraph trước, giữ nguyên mỗi điều khoản/mục trong cùng một chunk. Điều này giúp retrieval trả về chunk chứa đầy đủ ngữ cảnh pháp lý, thay vì bị cắt ngang giữa điều khoản.

**Parameter tuning (trên dieukhoandatcoc.md):**

| chunk_size | Chunk Count | Avg Length | Min | Max | Nhận xét |
|-----------|-------------|------------|-----|-----|----------|
| 300 | 30 | 168 | 1 | 300 | Quá nhỏ, nhiều chunk rời rạc |
| **500** | **16** | **317** | **1** | **494** | **Cân bằng tốt, mỗi chunk ~ 1 điều khoản** |
| 800 | 9 | 565 | 177 | 773 | Gộp nhiều điều, vẫn chấp nhận được |
| 1000 | 7 | 727 | 305 | 977 | Gộp quá nhiều, mất focus |

→ Chọn **chunk_size=500** là tối ưu cho domain pháp lý VinFast.

### So Sánh: Strategy của tôi vs Baseline

| Tài liệu | Strategy | Chunk Count | Avg Length | Retrieval Quality? |
|-----------|----------|-------------|------------|-------------------|
| dieukhoandatcoc.md | FixedSizeChunker (baseline) | 12 | 471 | ⚠️ Cắt ngang điều khoản, chunk thiếu context |
| | **RecursiveChunker (của tôi)** | **16** | **317** | **✅ Mỗi chunk chứa trọn 1-2 điều khoản** |
| chinhsachthuepin.md | FixedSizeChunker (baseline) | 33 | 500 | ⚠️ Cắt giữa Điều 2, 3, 4 |
| | **RecursiveChunker (của tôi)** | **44** | **337** | **✅ Giữ ranh giới Điều khoản** |

### So Sánh Với Thành Viên Khác

| Thành viên | Strategy | Retrieval Score (/10) | Điểm mạnh | Điểm yếu |
|-----------|----------|----------------------|-----------|----------|
| Tôi | RecursiveChunker (chunk_size=500) | | Giữ cấu trúc paragraph/điều khoản | Nhiều chunk hơn, min chunk có thể rất nhỏ |
| [Tên] | | | | |
| [Tên] | | | | |

**Strategy nào tốt nhất cho domain này? Tại sao?**
> *(Sẽ điền sau khi so sánh kết quả benchmark với nhóm)*
��a trên dấu hiệu gì?*
